...
public:
    ...
    bool isFlightOverlapping(  Flight& newFlight)   {
    ...
    void removeFlight(  string& flightNumber) {
        auto it = find_if(flights.begin(), flights.end(), [&flightNumber](  Flight& flight) {
    ...
    void displayFlightsSortedByDepartureTime()   {
        ...
        sort(sortedFlights.begin(), sortedFlights.end(), [](  Flight& a,   Flight& b) {
    ...
    do {
        ...
        switch (choice) {
            case 1: {
                ...
                cin >> dest;
                cout << "Enter departure time (HH): ";
                cin >> time;
                time+=":00";
                Flight a(number, dep, dest, time);
                system.addFlight(a);
                break;
            }
            case 2: {
